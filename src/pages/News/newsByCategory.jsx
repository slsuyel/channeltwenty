    ...
    <div className="row mx-auto my-3 container-fluid ">
      ...
      <div className="col-md-7 row  mx-auto">
        {data.length == 0 && (
          <div className="card">
            ...
            <RelatedNews
              slug={
                'obilmbe-gajay-zuddhbirtir-ahwan-janiye-niraptta-prishde-prstab-pas'
              }
            />
          </div>
        )}
        ...
        <Link
          to={`/news/${data[0]?.slug}`}
          className={data.length <= 0 ? 'd-none ' : ''}
        >
          <div className=" position-relative">
            ...
            <div className="position-absolute title-text">
              <h2 className="text-white">{data[0]?.title}</h2>
            </div>
          </div>
        </Link>
              ...
              >
                <div className="d-flex rounded newscard m-0 gap-2 ">
                  <div>
                    ...
                    <p
                      style={{ color: '#0004f4', fontSize: '14px' }}
                      className="mb-0"
                    >
                  ...
                  <div>
                    <img src={news.banner} alt="" className="common-image" />
                  </div>
                </div>
              </Link>
            </div>
        ...
        <div>
          <img
            ...
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_8OMgJX6-WJ09ITMYlmpiQB37tNxQz6fgVj2_xWAkAjuy-vj9IVkEe_b6xpjmA2BkZQ&usqp=CAU"
            alt=""
          />
        </div>

        {data.slice(7, 17).map(news => (
          <div
            key={news.id}
            className="col-md-3 my-1 d-none d-md-block d-lg-block"
          >
            <Link
              to={`/news/${news.slug}`}
              className="text-dark text-decoration-none "
      ...
      <div className="col-md-5 mx-auto">
        <div>
          ...
          {latestNews?.slice(0, 5).map((newsItem, index) => (
            ...
            >
              ...
              <div>
                ...
                >
                  <h6 className="fw-bold">{newsItem.title}</h6>
                  <p
                    style={{ color: '#0004f4', fontSize: '14px' }}
                    className="mb-0"
                  >
                    <i className="fas fa-clock me-1 opacity-75"></i>
                    {new Date(newsItem.date).toLocaleDateString('bn-BD', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                    })}
          ...
          <SideBarAdd
            img={
              'http://backend.newsnow24.com/storage/photos/shares/Ads/kishwan.gif'
            }
          />
          <NewsTab />
        </div>
