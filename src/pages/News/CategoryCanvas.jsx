...
const CategoryCanvas = () => {
  ...
  return (
    <div className="bg-body-secondary d-block d-sm-none me-2 text-end news-menubar-mobile">
      <button
        type="button"
        className="btn btn-danger rounded-0 text-end"
      ...
      <div
        className={`w-50 offcanvas offcanvas-start${
          isOffcanvasOpen ? ' show' : ''
        }`}
        tabIndex="-1"
        id="offcanvasExample"
        aria-labelledby="offcanvasExampleLabel"
        style={{ background: '#000028', zIndex: '999' }}
      >
        <div className="justify-content-around offcanvas-header p-0">
          <img
            src={logo}
            width={80}
            alt=""
            className="img-circle img-fluid m-2"
          />
          <button
            type="button"
        ...
        <div className="offcanvas-body mt-1">
          <ul className="list-unstyled">
            <li className="border-bottom border-secondary fs-5 mb-2 text-white">
              <Link
                className="fs-5 mb-2 text-center text-white text-decoration-none"
                to="/news"
              >
                প্রচ্ছদ
              </Link>
            ...
            {reArrCategories.map(category => (
              <li
                className="border-bottom border-secondary fs-5 mb-2 text-white"
                key={category.id}
              >
                {category.children && category.children.length > 0 ? (
                  <div className="dropdown">
                    <Link
                      className="dropdown-toggle fs-5 mb-2 text-center text-white text-decoration-none"
                      to="#"
                      role="button"
                    ...
                    <ul
                      className="dropdown-menu ps-2 bg-gradient"
                    ...
                    >
                      {category.children.map(subcategory => (
                        <li key={subcategory.id}>
                          <Link
                            className="border-bottom border-secondary fs-5 mb-2 text-center text-white text-decoration-none"
